...
public static void shortestPath(Graph graph, int source, int N)
{
    ...
    PriorityQueue<GraphNode> minHeap = new PriorityQueue<>((lhs, rhs) -> lhs.weight - rhs.weight);
    minHeap.add(new GraphNode(source, 0));
    ...
    while (!minHeap.isEmpty())
    {
        ...
        for (Edge edge: graph.adjList.get(u))
        {
            ...
            if (!done[v] && (dist.get(u) + weight) < dist.get(v))
            {
                ...
                prev[v] = u;
                minHeap.add(new GraphNode(v, dist.get(v)));
            }
        }
